...
We've seen front-end development with HTML, CSS< and JavaScript, and back-end development with JavaScript through Node.js and the Express framework.  Ruby on Rails is an earlier framework – one of the first frameworks to use the language of Ruby to build web application servers.
...
One of the things that's important to people who write code in Ruby is how the code _reads_. Rubyists are known for wanting beautiful code, and writing it in a way that reads as much like normal English as possible. That's part of what makes it great for beginners, is that it's more readable than many other languages.
...
> Ruby is a lot more forgiving than JavaScript when it comes to details like that. In the end you'll find you have an appreciation for both, but for now let's relish forgetting the ';'

## Data Types - Demo (15 mins)

**Question:** What data types have you guys been using in JavaScript? Let's write them out.

- **Booleans** are written as `true` and `false`
- **Numbers** are written as `12` or `9.45`
- **Strings** are written as `"awesome"`
- **null** and **undefined** are special cases
- **Arrays** (actually a kind of object) are written as `['x','y','z']`
...
- `nil`, roughly the equivalent of _null_ _(technically **NilClass**)_
...
- `{keys: ['some', 'values'] }` is called a **Hash**, but works almost the same
...
This is called "Type Coercion"; Ruby now knows that we want a Float back. We saw a similar effect in JavaScript: `"1" + 1` gives `"11"`.
...
Since you guys will be commenting your code (so that other developers can read it and understand why you wrote it how you did), that'll be useful.

#### Fun Tip: Ruby strings have a superpower!
...
That works, but string interpolation is way cooler:
...
It works with anything – any code can run in those brackets, and it'll evaluate and turn into a string. 
...
If you did that in JavaScript, you'd end up with a (probably unintended) global variable.
...
A local variable (which we usually write in lower_snake_case) is a quick placeholder, and gets forgotten as soon as your method or function is over.
...
These are great when you just need to temporarily store something or quickly give something a readable variable name, but won't need it later outside the method.

#### Instance Variable

An instance variable (also, by convention, lower_snake_case) is a variable that is defined in an instance of a Ruby object. We'll see Ruby objects later in the week, but remember an instance is just an example of an object, one thingy in the great world of things.
...
Instance variables let us store a variable once and use it however many methods we need inside an object.


#### Constant

Mostly, we're able to change what a variable's holding if we so choose – constants are designed for the opposite. Constants are meant to be placeholders that _never change_. We write their names in all capital letters.
...
Use what you just learned about Ruby data types, methods and string interpolation, hop in ```irb`` and get through as many of the following questions as you can in 15 minutes:
